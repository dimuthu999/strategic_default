...
for(fn in filelist) {
  tryCatch({
    ...
    lids <- unique(pefdata[pefdata$current_equity<=-0.2,]$loanid)
    lids <- lids[!is.na(lids)]
    pefdata <- pefdata[pefdata$loanid %in% lids, ]
    
    lids <- sqldf("select pd1.loanid,pd1.delstatus_t from pefdata as pd1
                  JOIN
                  (select loanid,min(reportingperiod_t) as start from pefdata where current_equity<=-0.2 group by loanid) as pd2
                  ON pd1.loanid=pd2.loanid AND pd1.reportingperiod_t=pd2.start")
    ...
    pefdata <- sqldf("select pd1.* from pefdata as pd1 
                     JOIN
                     (select loanid,min(reportingperiod_t) as start,min(reportingperiod_t)+366 as end from pefdata where current_equity<-0.2 group by loanid) as pd2
                     ON pd1.loanid=pd2.loanid AND pd1.reportingperiod_t>=pd2.start AND pd1.reportingperiod_t <= pd2.end 
    ...
    if(i==1)  {
      write.table(t(names(pefdata)),file="PMT_vs_Rent_freddie_cox_sample_Sep292016_20pct.csv",sep="|",col.names=FALSE,quote=FALSE,row.names = FALSE)
    }
    write.table(pefdata,file="PMT_vs_Rent_freddie_cox_sample_Sep292016_20pct.csv",append=TRUE,sep="|",col.names=FALSE,quote=FALSE,row.names = FALSE,na="")
