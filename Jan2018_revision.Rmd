...
## Split by price collapse size 1
```{r}
setwd(file_path)
...
## 3 Digit zip codes with similar price movements across 5-digit zip codes in 2008 - 2012
```{r}
...
felm_regs[[2]] <- felm(formula,data=temp[temp$corr_q1 >0.9,])
felm_regs[[3]] <- felm(formula,data=temp[temp$corr_median >0.9,])

# condf <- c("Cond. F. Stat",round(condfstat(felm_regs[[1]])[[1]],2),round(condfstat(felm_regs[[2]])[[1]],2),round(condfstat(felm_regs[[3]])[[1]],2),round(condfstat(felm_regs[[4]])[[1]],2),round(condfstat(felm_regs[[5]])[[1]],2))

printtable(felm_regs,c("p10 > 0.9","p25 > 0.9","Median > 0.9"),"default",note,"mtg30us",list("condf"))
```
