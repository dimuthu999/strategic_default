...
# 
# 
#   bw <- npregbw(formula=as.vector(temp$resid_default)~as.vector(temp$resid_mpay_value_fit),bws=0.75,bwtype="fixed",bandwidth.compute=F)
#   model <- npreg(bws=bw,gradients = TRUE)
#   plot.out <- plot(model, plot.errors.method="asymptotic",plot.errors.style="band",plot.behavior="data")
#   y.eval <- fitted(plot.out$r1)
...
# }
# 
# saveRDS(df_all,file="kernal_regs_2.rds")

df_all <- readRDS(file="kernal_regs_2.rds")
...
df_all <- df_all[df_all$x.eval>0.008 & df_all$x.eval<0.009,]
...
# }
# 
# saveRDS(df_all,file="kernal_regs_pe.rds")

df_all2 <- readRDS(file="kernal_regs_pe.rds")
...
df_all2 <- df_all2[df_all2$x.eval>0.008 & df_all2$x.eval<0.009,]
...
ggplot(df_comb,aes(x=x.eval, y=value,colour=ne_cutoff)) + geom_line(aes(linetype=ne_cutoff),size=0.75) +scale_linetype_manual(values = c(1,5,6,3,2,4))+scale_colour_manual(values=c("black","black","black","gray50","gray50","gray50"))+ theme_bw()+ylab("FE adjusted default rate") + xlab("FE adjusted Mortgage-to-value(fit)")+ theme(legend.position="bottom",legend.title=element_blank(),legend.box = "horizontal")
```
