...
fn_cox = "temp_cox_strategicdefault_new.csv"
...
hpi$zip <- hpi$zip*100
hpi['current_q'] <- (as.yearqtr(as.Date(paste(hpi$year,hpi$qt*3,"01",sep="-"))))
hpi$qt <- NULL
hpi$year <- NULL
...
unemp['ne_qt'] <- (as.yearqtr(unemp$reportingperiod_t))
...
generate_sample <- function(cut)  {
  ...
  for(fn in filelist) {
    tryCatch({
      ...
      lids <- sqldf(paste("select pd1.loanid,pd1.delstatus_t from pefdata as pd1 JOIN (select loanid,min(reportingperiod_t) as start from pefdata where current_equity<=",ne_cut," and current_equity >",(ne_cut-0.1)," group by loanid) as pd2 ON pd1.loanid=pd2.loanid AND pd1.reportingperiod_t=pd2.start",sep=""))
      ...
      #temp <- sqldf("select loanid,min(reportingperiod_t) as start,min(reportingperiod_t)+366 as end,current_equity from pefdata where current_equity<=-0.2 and current_equity > -0.3 group by loanid")
      
      # pefdata_prv_equity <- sqldf(paste("select pd1.* from pefdata as pd1
      #                        JOIN
      #                        (select loanid,min(reportingperiod_t) -150 as start,min(reportingperiod_t)-10 as end from pefdata where current_equity<=",ne_cut,"  group by loanid) as pd2 ON pd1.loanid=pd2.loanid AND pd1.reportingperiod_t>=pd2.start AND pd1.reportingperiod_t <= pd2.end where pd1.delstatus_t in (0,1)",sep=""))
      # pefdata_prv_equity <- aggregate(pefdata_prv_equity$current_equity, by=list(pefdata_prv_equity$loanid), FUN = function(x) mean(x,na.rm = TRUE))
      # names(pefdata_prv_equity) <- c("loanid","prev_equity")
      
      pefdata <- sqldf(paste("select pd1.* from pefdata as pd1 
                             JOIN
                             (select loanid,min(reportingperiod_t)-10 as start,min(reportingperiod_t)+366 as end from pefdata where current_equity<=",ne_cut," and current_equity >",(ne_cut-0.1)," group by loanid) as pd2 ON pd1.loanid=pd2.loanid AND pd1.reportingperiod_t>=pd2.start AND pd1.reportingperiod_t <= pd2.end where pd1.delstatus_t in (0,1)",sep=""))
      pefdata_prv_equity <- pefdata_prv_equity[!duplicated(pefdata_prv_equity$loanid),]
      ...
      #pefdata <- merge(pefdata,pefdata_prv_equity,by="loanid",all.x = TRUE)
  ...
  fn_cox_out = paste("cox_ne_",ne_cut,"_Dec082016.rds",sep="")
