...
cox_formula_1 <- as.formula("Surv(duration,def)~adjusted_mpay_rent+interestrate+median_forc_time+age_t+factor(ne_year)+factor(recourse)+factor(ne_cutoff)+log(upb)/ltv+I(interestrate==interestrate_t)")

cox_formula_2 <- as.formula("Surv(duration,def)~adjusted_mpay_rent+median_forc_time+interestrate+age_t+age_t*age_t+del_unemp+factor(ne_year)+fico+ltv+dti+medhhincome+medhouseage+vacantpct+bachelorpct+withsalarypct+factor(occupancy)+factor(loanpurpose)+factor(state)+factor(ne_cutoff)+log(upb)/ltv+I(interestrate==interestrate_t)")

cox <-list()
cox[[1]] <- coxph(cox_formula_1,data = zillow_cox)
cox[[2]] <- coxph(cox_formula_2,data = zillow_cox)

stargazer(cox, omit = c("ne_year","loanpurpose","occupancy","ne_cutoff","state"),omit.labels = c("Year","","","",""),type = output.type,no.space=TRUE)
```
...
cox_formula <- as.formula("Surv(duration,def)~adjusted_mpay_rent+I(adjusted_mpay_rent^2)+I(adjusted_mpay_rent^3)+I(adjusted_mpay_rent^4)+I(adjusted_mpay_rent^5)+median_forc_time+age_t+I(median_forc_time^2)+I(median_forc_time^3)+I(median_forc_time^4)+I(median_forc_time^5)+interestrate+age_t*age_t+del_unemp+factor(ne_year)+fico+ltv+dti+medhhincome+medhouseage+vacantpct+bachelorpct+withsalarypct+log(upb)+factor(occupancy)+factor(loanpurpose)+factor(recourse)+factor(ne_cutoff)+log(upb)/ltv+I(interestrate==interestrate_t)")
...
cox_formula_1 <- as.formula("Surv(duration,selfcure)~adjusted_mpay_rent+median_forc_time+current_equity+age_t+factor(ne_year)+factor(recourse)+log(upb)/ltv+interestrate+I(interestrate==interestrate_t)")

cox_formula_2 <- as.formula("Surv(duration,selfcure)~adjusted_mpay_rent+median_forc_time+current_equity+age_t+age_t*age_t+del_unemp+factor(ne_year)+fico+ltv+dti+factor(occupancy)+factor(loanpurpose)+factor(recourse)+log(upb)/ltv+interestrate+I(interestrate==interestrate_t)")
