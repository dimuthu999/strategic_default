...
cox_formula_1 <- as.formula("Surv(duration,selfcure)~adjusted_mpay_rent+median_forc_time+age_t+factor(ne_year)+factor(recourse)+log(upb)/ltv")

cox_formula_2 <- as.formula("Surv(duration,selfcure)~adjusted_mpay_rent+median_forc_time+age_t+age_t*age_t+del_unemp+factor(ne_year)+fico+ltv+dti+medhhincome+medhouseage+vacantpct+bachelorpct+withsalarypct+factor(occupancy)+factor(loanpurpose)+factor(recourse)+log(upb)/ltv")
...
```{r results='asis',warning=FALSE}
cox_formula <- as.formula("Surv(duration,selfcure)~adjusted_mpay_rent+I(adjusted_mpay_rent^2)+I(adjusted_mpay_rent^3)+I(adjusted_mpay_rent^4)+I(adjusted_mpay_rent^5)+median_forc_time+age_t+age_t*age_t+del_unemp+factor(ne_year)+fico+ltv+dti+medhhincome+medhouseage+vacantpct+bachelorpct+withsalarypct+factor(occupancy)+factor(loanpurpose)+factor(recourse)+log(upb)/ltv")
