...
### Default % (Matched 10% vs - 10%)
```{r defpctmatched10}
setwd(file_path)
pos_eqty <- readRDS("cox_ne_0.1_Dec082016.rds")
pos_eqty <- pos_eqty[,c("loanid","ne_cutoff","def","fico","upb","interestrate","zip","ne_qt")]
pos_eqty <- pos_eqty[!duplicated(pos_eqty[,c("loanid","ne_cutoff")]),]
temp  <- zillow_cox[zillow_cox$ne_cutoff==-0.1,]
temp <- temp[!duplicated(temp[,c("loanid","ne_cutoff")]),]
temp <- temp[!is.na(temp$adjusted_mpay_rent),]
temp <- temp[,c("loanid","ne_cutoff","def","fico","upb","interestrate","zip","ne_qt")]
pos_eqty <- rbind(pos_eqty,temp)
...
MatchBalance(Tr~def+fico+interestrate+upb,match.out = propensity,nboots = 0,data=pos_eqty,paired = TRUE)

rm(pos_eqty)
```

### Default % (Matched 20% vs - 20%)
```{r defpctmatched20, echo=FALSE}
setwd(file_path)
...
### Default % (Matched 50% vs - 50%)
```{r defpctmatched50, echo=FALSE}
setwd(file_path)
...
---------------------------------------------------
Weak instruments|  6   58320   2664.73  < 2e-16 ***
Wu-Hausman      |  1   58324     43.26 4.84e-11 ***
```{r tsls}
temp <- zillow_cox[zillow_cox$ne_cutoff==-0.1,]
temp <- temp[!duplicated(temp[,c("loanid","ne_cutoff")]),]
...
Diagnostic tests:
                  Test                                              | df1  df2   statistic p-value ----------------------------------------------------------------------------------------------------
Weak instruments (adjusted mpay rent)                               |   6 58320  2664.729  <2e-16 ***
Weak instruments (I(adjusted mpay rent > 1)TRUE)                    |   6 58320  1249.276  <2e-16 ***
Weak instruments (adjusted mpay rent:I(adjusted mpay rent > 1)TRUE) |   6 58320  1887.863  <2e-16 ***
Wu-Hausman                                                          |   3 58320    26.490  <2e-16 ***
```{r tslsinteraction,echo=FALSE}
temp <- zillow_cox[zillow_cox$ne_cutoff==-0.1,]
temp <- temp[!duplicated(temp[,c("loanid","ne_cutoff")]),]
temp['org_qt']<-as.yearqtr(as.Date(as.character(temp$orgdate)))
temp = temp[temp$loanterm==360,]
temp$def <- ifelse(temp$adjusted_mpay_rent >= 0.8 & temp$adjusted_mpay_rent < 1,0,temp$def)
temp['current_hv_cat'] <- temp$current_housevalue %/% 25000
temp['zip_year'] <- paste(temp$zip,temp$ne_year,sep="_")
...
iv_reg[[1]] <- lm(def ~ adjusted_mpay_rent + fico + ltv+ dti + log(upb) + current_hv_cat+interest_diff *factor(zip_year), data=temp)
se[[1]] <- NULL
iv_reg[[2]] <- lm(adjusted_mpay_rent~interestrate_avg+age_t+fico + current_hv_cat+ltv+ dti + log(upb) + interest_diff + factor(zip_year) , data=temp)
se[[2]] <- NULL
temp['mpay_rent_hat'] <- predict(iv_reg[[2]],temp)
iv_reg[[3]] <- lm(def ~ mpay_rent_hat*I(mpay_rent_hat>1) + fico + ltv+ dti + log(upb) + current_hv_cat+interest_diff +factor(zip_year),data=temp)
se[[3]] <- NULL #sqrt(diag(vcovHAC.default(iv_reg[[3]],sandwich = TRUE)))
iv_reg[[4]] <- ivreg(def ~ adjusted_mpay_rent*I(adjusted_mpay_rent>1) + fico + ltv+ dti + log(upb) + current_hv_cat+interest_diff +factor(zip_year) | fico + ltv+ dti + log(upb) +current_hv_cat+ interestrate_avg+ interest_diff + age_t +factor(occupancy) + factor(purpose)+factor(zip_year), data=temp)
se[[4]] <- NULL
stargazer(iv_reg,type = output.type,no.space = TRUE,omit = c("zip_year","purpose","occupancy"),omit.labels = c("zip * year","purpose","occupancy"),column.labels   = c("OLS","First Stage", "Second Stage"),   column.separate = c(1,1,2),omit.stat = c("f","ser","rsq"))
```



### Two Stage Least Squres - mpay/rent <1

```{r tslslt1, echo=FALSE}
temp <- zillow_cox[zillow_cox$ne_cutoff==-0.1 & zillow_cox$adjusted_mpay_rent<1,]
temp$def <- ifelse(temp$adjusted_mpay_rent >= 0.7,0,temp$def)
temp <- temp[!duplicated(temp[,c("loanid","ne_cutoff")]),]
temp['org_qt']<-as.yearqtr(as.Date(as.character(temp$orgdate)))
temp = temp[temp$loanterm==360,]
temp['current_hv_cat'] <- temp$current_housevalue %/% 25000
...
glm_prop <- glm(Tr~fico+log(upb)+interestrate,data=pos_eqty,family = 'binomial')
propensity <- Matchby(Y=Y,Tr=Tr,by=list(pos_eqty$zip),X=glm_prop$fitted.values,M=1,replace = FALSE,print.level = 0)
