...
setwd(file_path)
org  <- zillow_cox[zillow_cox$ne_cutoff==-0.01,]
...
for(th in c(1,2,5)) {
  ...
  propensity <- Matchby(Y=Y,Tr=Tr,by=list(data$zip,data$ne_qt),X=glm_prop$fitted.values,print.level = 0,replace = TRUE,M=1)
 ...
 data['current_hv_cat'] <- data$current_housevalue %/% 25000
  ...
  tryCatch({propensity <- Matchby(Y=Y,Tr=Tr,by=list(data$current_hv_cat),X=glm_prop$fitted.values,print.level = 0,replace = replace.value,M=M)},error=function(cond) {})
  #tryCatch({record[4]<-temp[[3]]$getElementText()},error=function(cond) {})
#   if(th==2) {
#     neg_eqty = org
#   } else 
  neg_eqty <- data[propensity$index.control,]
...
for(th in c(1,2,5)) {
 # cat(th)
  ...
  glm_prop <- glm(Tr~fico+log(upb)+interestrate+current_hv_cat,data=data,family = 'binomial')
  propensity <- Matchby(Y=Y,Tr=Tr,by=list(data$zip,data$ne_qt,data$current_hv_cat),X=glm_prop$fitted.values,print.level = 0,replace = TRUE,M=5)
 ...
 data['current_hv_cat'] <- data$current_housevalue %/% 25000
  ...
  glm_prop <- glm(Tr~fico+log(upb)+interestrate,data=data,family = 'binomial')
  tryCatch({propensity <- Matchby(Y=Y,Tr=Tr,by=list(data$current_hv_cat),X=glm_prop$fitted.values,print.level = 0,replace = replace.value,M=1)},error=function(cond) {})
  #tryCatch({record[4]<-temp[[3]]$getElementText()},error=function(cond) {})
#   if(th==2) {
#     neg_eqty = org
#   } else 
