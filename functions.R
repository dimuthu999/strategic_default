...
detsummarystats <- function(x, y, col){
  za <- ddply(x, y, .fun = function(xx){
    c(obs = length(xx[!is.na(xx[,col]),col]),
      mean =  winsor.mean(xx[,col], trim=0.05,na.rm = TRUE), 
      stddev = winsor.sd(xx[,col], trim=0.05, na.rm = TRUE),
      p05 = quantile(xx[,col], c(.05), na.rm = TRUE),
      p10 = quantile(xx[,col], c(.10), na.rm = TRUE),
      p25 = quantile(xx[,col], c(.25), na.rm = TRUE),
      median = quantile(xx[,col], c(.50), na.rm = TRUE),
      p75 = quantile(xx[,col], c(.75), na.rm = TRUE),
      p90 = quantile(xx[,col], c(.90), na.rm = TRUE),
      p95 = quantile(xx[,col], c(.95), na.rm = TRUE),
      max = max(xx[,col])) })
  is.num <- sapply(za, is.numeric)
  za[is.num] <- lapply(za[is.num], round, 4)
  return(za)
}

meansum <- function(x, y, col){
  za <- ddply(x, y, .fun = function(xx){
    c(mean =  winsor.mean(xx[,col],trim=0.02, na.rm = TRUE)) })
  is.num <- sapply(za, is.numeric)
  za[is.num] <- lapply(za[is.num], round, 4)
  return(za)
}

meansum2 <- function(x, y, col){
  za <- ddply(x, y, .fun = function(xx){
    c(mean =  mean(xx[,col], na.rm = TRUE)) })
    return(za)
}

meansdmedsum <- function(x, y, col){
  za <- ddply(x, y, .fun = function(xx){
    c(mean =  winsor.mean(xx[,col],trim=0.02, na.rm = TRUE),
      stddev = winsor.sd(xx[,col], trim=0.02, na.rm = TRUE),
      median = quantile(xx[,col], c(.50), na.rm = TRUE),
      obs = length(xx[!is.na(xx[,col]),col])) })
  is.num <- sapply(za, is.numeric)
  za[is.num] <- lapply(za[is.num], round, 4)
  return(za)
}

meansdmedsum2 <- function(x, y, col){
  za <- ddply(x, y, .fun = function(xx){
    c(mean =  mean(xx[,col],na.rm = TRUE),
      stddev = sd(xx[,col], na.rm = TRUE),
      median = quantile(xx[,col], c(.50), na.rm = TRUE),
      obs = length(xx[!is.na(xx[,col]),col])) })
  is.num <- sapply(za, is.numeric)
  za[is.num] <- lapply(za[is.num], round, 4)
...
as.year <- function(x) as.numeric(floor(x))
