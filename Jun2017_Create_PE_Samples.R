...
generate_sample <- function(cut)  {
    ...
    tryCatch({
      ...
      if(ne_cut>0) {
        pefdata <- sqldf(paste("select pd1.* from pefdata as pd1 
                             JOIN
                             (select loanid,min(cd)-10 as start from pefdata where current_equity>=",(ne_cut+0.1)," group by loanid) as pd2 ON pd1.loanid=pd2.loanid AND pd1.cd>=pd2.start",sep=""))
      }
            
      # lids <- sqldf(paste("select loanid,min(cd) as start from pefdata where current_equity<=",ne_cut," ",sep=""))
      lids <- sqldf(paste("select pd1.loanid,pd1.delstatus_t from pefdata as pd1 JOIN (select loanid,min(cd) as start from pefdata where current_equity<=",ne_cut," group by loanid) as pd2 ON pd1.loanid=pd2.loanid AND pd1.cd=pd2.start",sep=""))
      ...
      pefdata <- sqldf(paste("select pd1.* from pefdata as pd1 
                             JOIN
                             (select loanid,min(cd)-10 as start,min(cd)+366 as end from pefdata where current_equity<=",ne_cut," group by loanid) as pd2 ON pd1.loanid=pd2.loanid AND pd1.cd>=pd2.start AND pd1.cd <= pd2.end where pd1.delstatus_t in (0,1)",sep=""))
  ...
  fn_cox_out = paste("cox_ne_",ne_cut,"_Jun072017_60day.rds",sep="")
